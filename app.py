...
if sidebarOptions == 'PNL':
    ...
    pnl_by_desig_agg = pnl.groupby("date")["estPnl"].sum().reset_index()
    ...
    pnl_by_book_agg = pnl.groupby("date")["estPnl"].sum().reset_index()
    ...
    pnl_by_book_timeseries.add_scatter(x = pnl_by_book_agg["date"], y = pnl_by_book_agg["estPnl"], mode = "lines", name = "Total Pnl - All Books")
...
if sidebarOptions == 'Risk':
    ...
    st.header("Risk by Isin")
    isin_selection = st.selectbox("Select an Isin", isins)
    risk_filtered = risk[risk["isin"] == isin_selection]
    risk_timeseries = px.line(risk_filtered, x = "time", y =["bondRisk", "etfRisk", "totalRisk"])
    st.plotly_chart(risk_timeseries)

    st.header("Risk by Maturity")
    ...
    st.plotly_chart(risk_maturity_bar)

    st.header("Risk by Asset")
    ...
    st.plotly_chart(risk_asset_bar)
    
    st.header("Risk by Issuer")
    ...
    st.plotly_chart(risk_issuer_bar)

    st.header("Risk by Rating")
    ...
    st.plotly_chart(risk_rating_bar)
